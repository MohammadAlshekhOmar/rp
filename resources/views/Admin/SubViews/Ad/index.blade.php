                    ...
                    <div class="table-responsive text-center">
                        <table class="table table-striped table-head-custom table-checkable" id="dtTable">
                            ...
                            <tbody id="add-row">
                                @foreach ($ads as $ad)
                                    <tr id="sid{{ $ad->id }}">
                                        <td><span class="badge rounded-pill badge-light-primary">{{ $ad->id }}</span>
                                        </td>
                                        <td><img src="{{ $ad->image }}" alt="ad image" style="width:75px;height:75px;">
                                        <td>{{ $ad->provider?->name }}</td>
                                        </td>
                                        <td>{{ $ad->text }}</td>
                                        <td>{{ $ad->ad_status?->name }}</td>
                                        <td>
                                            <div class="d-flex flex-column align-items-center">
                                                <div class="form-check form-switch form-check-primary">
                                                    <input type="checkbox" class="form-check-input" name="changeStatus"
                                                        id="{{ $ad->id }}"
                                                        {{ $ad->deleted_at ? null : 'checked=checked' }} />
                                                </div>
                                            </div>
                                        </td>
                                        <td><span
                                                class="badge rounded-pill badge-light-secondary">{{ $ad->created_at }}</span>
                                        </td>
                                        <td>
                                            @if ($ad->ad_status_id == App\Enums\AdStatusEnum::PENDING->value)
                                                <a href="javascript:;" class="btnAccept" id={{ $ad->id }}
                                                    data-bs-toggle="tooltip" title="" data-bs-animation="false"
                                                    data-bs-original-title="قبول">
                                                ...
                                                <a href="javascript:;" class="btnReject" id={{ $ad->id }}
                                                    data-bs-toggle="tooltip" title="" data-bs-animation="false"
                                                    data-bs-original-title="رفض">
                                                    <svg style="margin:12px;" xmlns="http://www.w3.org/2000/svg"
                                                        width="24" height="24" viewBox="0 0 24 24" fill="none"
                                                        stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                            ...
                                            <a href="javascript:;" class="btnDelete" id={{ $ad->id }}
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        ...
                                        <img id="show_image" src="#"
                                            style="display:none;width:150px;height:150px;" alt="ad image" />
                                    </div>
                                </div>
                            </div>
                        </div>
...
@section('page-script')
    ...
    <script>
        ...
        $("#editModal").on("hidden.bs.modal", function() {
            $("[name='provider_id']").select2().val('').trigger("change");
            $("[name='ar[text]']").val('');
            $("[name='en[text]']").val('');
        ...
        $('#editModal').on('shown.bs.modal', function() {
            $.ajax({
                ...
                success: function(response, textStatus, xhr) {
                    ...
                    $("[name='provider_id']").select2().val(response.data.provider_id).trigger(
                        "change");
                    $("[name='ar[text]']").val(response.data.ar.text);
                    $("[name='en[text]']").val(response.data.en.text);
                    ...
                    if (xhr.status == 201) {
                        ...
                        $('#add-row').prepend(`
                            <tr id="sid${response.data.id}">
                                ...
                                <td><img src="${response.data.image}" alt="ad image" style="width:75px;height:75px;"></td>
                                <td>${response.data.provider.name}</td>
                                <td>${response.data.text}</td>
                                <td>${response.data.ad_status.name}</td>
                                <td>
                                    <div class="d-flex flex-column align-items-center">
                                        <div class="form-check form-switch form-check-primary">
                                            <input type="checkbox" class="form-check-input" name="changeStatus"
                                ...
                                <td>
                                    <a href="javascript:;" class="btnAccept" id=${response.data.id}
                                        data-bs-toggle="tooltip" title="" data-bs-animation="false"
                                        data-bs-original-title="قبول">
                                        <svg style="margin:12px;" xmlns="http://www.w3.org/2000/svg"
                                            width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    ...
                                    <a href="javascript:;" class="btnReject" id=${response.data.id}
                                        data-bs-toggle="tooltip" title="" data-bs-animation="false"
                                        data-bs-original-title="رفض">
                                        <svg style="margin:12px;" xmlns="http://www.w3.org/2000/svg"
                                            width="24" height="24" viewBox="0 0 24 24" fill="none"
            ...
            $.ajax({
                ...
                success: function(response, textStatus, xhr) {
                    ...
                    $('#sid' + response.data.id).find("td").eq(1).html(
                        `<img src="${response.data.image}" alt="ad image" style="width:75px;height:75px;">`
                    );
                    $('#sid' + response.data.id).find("td").eq(2).html(response.data.provider.name);
        ...
        $(document).on('click', '.btnAccept', function() {
            Swal.fire({
                title: 'هل أنت متأكد من عملية القبول؟',
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        success: function(response, textStatus, xhr) {
                            ...
                            fireMessage('حسناً', 'تم القبول بنجاح');
        ...
        $(document).on('click', '.btnReject', function() {
            Swal.fire({
                title: 'هل أنت متأكد من عملية الرفض؟',
                text: 'لن تتمكن من التراجع!',
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        success: function(response, textStatus, xhr) {
                            ...
                            fireMessage('حسناً', 'تم الرفض بنجاح');
                            window.location.reload();
                        },
                        error: function(response) {
                            if (response.status == 401) {
