...
@section('content')
    ...
    <section id="basic-datatable">
        <a href="{{ route('admin.providers.showAdd') }}" class="btn btn-relief-primary mb-2">
            <span>إضافة</span>
            <i data-feather="plus" class="me-25"></i>
        </a>
        {{-- <a href="{{ route('admin.providers.export') }}" class="btn btn-relief-primary mb-2">
            <span>تصدير</span>
                        ...
                        <table class="table table-striped table-head-custom table-checkable" id="dtTable">
                            ...
                            <tbody id="add-row">
                                @foreach ($providers as $provider)
                                    <tr id="sid{{ $provider->id }}">
                                        <td><span class="badge rounded-pill badge-light-primary">{{ $provider->id }}</span>
                                        </td>
                                        <td>{{ $provider->name }}</td>
                                        ...
                                        <td>
                                            @foreach ($provider->regions as $region)
                                                <span
                                                    class="badge rounded-pill badge-light-info">{{ $region->region->name }}</span>
                                            @endforeach
                                        </td>
                                        <td>{{ $provider->commercial_register }}</td>
                                            ...
                                            <div class="d-flex flex-column align-items-center">
                                                <div class="form-check form-switch form-check-primary">
                                                    <input type="checkbox" class="form-check-input" name="changeStatus"
                                                        id="{{ $provider->id }}"
                                                        {{ $provider->deleted_at ? null : 'checked=checked' }} />
                                                </div>
                                            </div>
                                        </td>
                                        <td><span
                                        ...
                                        <td>
                                            ...
                                            <a href="{{ route('admin.providerRates.index', ['provider_id' => $provider->id]) }}"
                                                data-bs-toggle="tooltip" title="" data-bs-animation="false"
                                            ...
                                            <a href="{{ route('admin.providers.showEdit', ['id' => $provider->id]) }}"
                                                data-bs-toggle="tooltip" title="" data-bs-animation="false"
                                            ...
                                            <a href="javascript:;" class="btnDelete" id={{ $provider->id }}
